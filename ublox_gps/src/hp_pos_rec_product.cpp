...
void HpPosRecProduct::callbackNavRelPosNed(const ublox_msgs::msg::NavRELPOSNED9 &m) {
  ...
  if (getRosBoolean(node_, "publish.nav.heading")) {
    ...
    double heading = - (static_cast<double>(m.rel_pos_heading) * 1e-5 / 180.0 * M_PI) - M_PI; // NOTICE(erik): Changed from M_PI_2 -> M_PI
    ...
    if (m.flags & ublox_msgs::msg::NavRELPOSNED9::FLAGS_REL_POS_HEAD_VALID) {
      imu_.orientation_covariance[8] = ::pow(m.acc_heading / 10000.0, 2) / 3283.0; // NOTICE(erik): Division with / 3283.0
    }
